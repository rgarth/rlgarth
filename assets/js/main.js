...
const counts = isMobile ? {
    bgLeaves: 10, midLeaves: 15, fgLeaves: 8, flowers: 4, trunks: 8, webs: 4, bones: 2
} : {
    bgLeaves: 30, midLeaves: 40, fgLeaves: 25, flowers: 10, trunks: 20, webs: 12, bones: 5
};
...
const skyStops = [
    { pos: 0, color: '#f8f4ef' }, { pos: 0.2, color: '#e8f0e8' }, { pos: 0.35, color: '#d0e0d0' },
    { pos: 0.45, color: '#4a5060' }, { pos: 0.55, color: '#1a1a28' }, { pos: 0.65, color: '#1a1a28' },
    { pos: 0.75, color: '#5a6070' }, { pos: 0.85, color: '#e0e8e0' }, { pos: 1, color: '#f8f4ef' }
];
...
function update() {
    ...
    const darkStart = 0.38, darkPeak = 0.55, darkEnd = 0.72;
    const inDarkZone = scrollPercent > darkStart && scrollPercent < darkEnd;
    
    let darkIntensity = 0;
    if (scrollPercent >= darkStart && scrollPercent <= darkPeak) darkIntensity = (scrollPercent - darkStart) / (darkPeak - darkStart);
    else if (scrollPercent > darkPeak && scrollPercent <= darkEnd) darkIntensity = 1 - (scrollPercent - darkPeak) / (darkEnd - darkPeak);
    ...
    foliageElements.forEach(item => {
        ...
        if (item.isDark) {
            ...
            scale = 0.3 + darkIntensity * item.z * zoomMultiplier;
            ...
            scale = 1 + scrollPercent * item.z * (isMobile ? 0.1 : 0.2);
    ...
    sections.forEach((sec, index) => {
        ...
        if (index === sections.length - 1 && scrollPercent > 0.85) isActive = true;
...
function spawnFairy() {
    if (fairyActive || !isInPenelopeSection()) { if (isInPenelopeSection()) scheduleNextFairy(); return; }
    fairyActive = true;
...
window.addEventListener('scroll', () => {
    ...
    if (inSection && !wasInSection) fairyTimeout = setTimeout(spawnFairy, 1000);
    else if (!inSection && wasInSection) stopFairies();
...
function spawnWraith() {
    if (wraithActive || !isInDarkSection()) { if (isInDarkSection()) scheduleNextWraith(); return; }
    wraithActive = true;
    ...
    wraith.className = `wraith ${goingLeft ? 'right-to-left' : 'left-to-right'} ${scaleType}`;
...
window.addEventListener('scroll', () => {
    ...
    if (inDark && !wasInDark) wraithTimeout = setTimeout(spawnWraith, 1500);
    else if (!inDark && wasInDark) stopWraiths();
    wasInDark = inDark;
});
...
function openExcerpt(excerptName) {
    ...
    // Load the excerpt content
    fetch(`assets/content/excerpt-${excerptName}.html`)
        .then(r => r.text())
        .then(html => {
            content.innerHTML = html;
            // Apply dark background if it's a dark excerpt
